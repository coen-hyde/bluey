...
Base._maskToProperties = function(properties) {
  if (_.isArray(properties)) return properties;

  // Do we want all properties?
  if (properties === 'all') {
  ...
  // Are we asking for a mask defined in the model?
  if (_.isString(properties) && this.masks && this.masks[properties]) {
    return this.masks[properties];
  }

  // Are we asking for an inverted mask?
  let invertedMask = (
    _.isString(properties) &&
    properties.substr(0, 1) === "!" &&
    this.masks &&
    this.masks[properties.substr(1)]
  );

  if (invertedMask) {
    var mask = properties.substr(1);
    var maskProperties = this.masks[properties];
    var allProperties = _.keys(this.properties);
    return _.without(allProperties, maskProperties);
  }
