...
Base.batchInsert = function(data, options, cb) {
  ...
  function batchValidate(data, cb) {
    if (options['disableValidation']) return cb(null, data);

    async.map(data, function(record, next) {
      model.validate(record, function(err, result, data) {
        if (!result.valid) return cb(result.errors);
        next(err, data);
      });
    }, cb);
  ...
  batchValidate(data, function(err, data) {
    if (err) return cb(err);
